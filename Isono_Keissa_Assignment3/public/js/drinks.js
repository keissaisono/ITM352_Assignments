...
*/
for (let i = 0; i < products.drinks.length; i++) {
    document.querySelector('.row').innerHTML += 
        `<div class="col-md-6 product_name mb-4">
        <div class="card">
            <div class="text-center">
                <img src="${products.drinks[i].image}" class="card-img-top border-top" alt="Product Image">
            </div>
            <div class="card-body">
                <h5 class="card-title">${products.drinks[i].name}</h5>
                <p class="card-text">
                    Price: $${(products.drinks[i].price).toFixed(2)}<br>
                    Available: ${products.drinks[i].qty_available}<br>
                    <!-- Total Sold: ${products.drinks[i].total_sold} -->
                </p>
...
//Executes to produce a validation message.
    function validateQuantity(quantity){
        ...
        }else if(quantityNumber > products.drinks[quantity.id]['qty_available']){
